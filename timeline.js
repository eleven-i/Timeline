...
function Timeline(domID, data, conf) {
  if (conf.sortLines) sortLines(data, conf);
  ...
  $(domID).bind("plothover", function (event, pos, item) {
    if (item) {
      if (previousPoint != item.dataIndex) {
        ...
        if (tooltipHtml) showTooltip(item.pageX, item.pageY, tooltipHtml);
      }
    } else {
      $("#timelineTooltip").fadeOut(400, function () {
        this.remove();
      });
      previousPoint = null;
    }
...
*/
  ...
  if (conf.lines === undefined) conf.lines = [];
  else conf.lines = conf.lines.slice(0);

  var scores = [];
  for (i = 0; i < conf.lines.length; ++i) scores[i] = 0;
  ...
  for (i = 0; i < data.length; ++i) {
    ...
    if (conf.sortLines.categories) {
      ...
      if (s >= 0) {
        ...
        scores[lineIndex] += Math.pow(
          data.length,
          conf.sortLines.categories.length - s - 1
        );
      }
    }
  ...
  var order = [];
  for (i = 0; i < scores.length; ++i) order[i] = i;
  ...
  for (i = 0; i < conf.lines.length; ++i) conf.lines[i] = oldLines[order[i]];
...
function makeFlotData(data, conf) {
  ...
  function getLineIndex(line) {
    ...
    if (y >= 0) return y;
    else return conf.lines.push(line) - 1;
  }
  ...
  for (i = 0; i < data.length; ++i) {
    ...
    } else tooltips[iCategory].push(data[i].label);
...
function makeFlotConfig(conf) {
  var tz = "browser";
  if (conf.timezone !== undefined) tz = conf.timezone;
  ...
  if (conf.lineLabels) {
    ...
    for (iLine = 0; iLine < conf.lines.length; iLine++)
      if (
        iLine < conf.lineLabels.length &&
        conf.lineLabels[iLine] !== undefined
      )
        d.yaxis.ticks.push([iLine, conf.lineLabels[iLine]]);
      // Run out of labels, try using the line index itself (as a string)
      else d.yaxis.ticks.push([iLine, "" + conf.lines[iLine]]);
  } else {
    // disable labeling
    d.yaxis.tickSize = 1;
    d.yaxis.tickFormatter = function (val, axis) {
      return "";
    };
  }
...
function showTooltip(x, y, contents) {
  $('<div id="timelineTooltip">' + contents + "</div>")
    .css({
      position: "absolute",
      display: "none",
      top: y + 5,
